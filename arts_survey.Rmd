...
We can also consider the technology tools available to students. This includes the various software, computers, cameras, and so on that can be made available for use in the arts. We do not have data on musical instruments or props, although it is certainly the case that those resources are useful in running an effective arts program in music and theater.  
...
Smartboards and laptops are the most frequent tools. In my time in school, I never took an art class that used smartboards or laptops, so I am suspicious that these tools have been included erroneously by respondents.  
To be expected, the least frequent tools are entire suites of equipment: interactive distance exchange labs, darkrooms and TV studios.  
Moving up the list, we find expensive pieces such as film cameras, keyboards and lighting equipment. Contrary to my expectations, various generic software packages: music and video editing and animation software, are also not particularly prevalent. These are expensive but I can imagine that Adobe or Microsoft could garner some goodwill by donating licenses to schools for proprietary tools. Otherwise, there are free tools out there, so this could indicate a lack of technological sophistication in the education system.  
If there is no benefit to having any of these tools, then efforts to increase their availability 

```{r tools stats}
df %>% dplyr::select(tools, perc_34_4_2018_ela) %>% 
  lm(perc_34_4_2018_ela~., data=.) %>% 
  summary()
# Animation software has a significant effect.
# color printers, digital tablets, _19 film cameras have a lesser but still significant effect.
