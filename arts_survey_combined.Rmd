...
Fitting a linear regression, for either English or math performance, to having a screened arts program does not yield a statistically significant coefficient. This holds true when we control for school demographic data, too.  

## Part Eight

### Middle school arts sequence

Some middle schools offer a three-year sequence of instruction in a single arts discipline. This could be a major differentiating factor in the arts education of participating students, as opposed to a single music class in seventh-grade. Since our data is on the school-level we cannot control for participation in these programs for individual students. It may be doubtful that the presence of such sequences would have a significant effect on an entire school, but we can still have a look.  

```{r 3yr sequence setup}
q22 <- df %>% colnames() %>% .[grepl('Q22', .)] %>% .[grepl('_C1', .)]
df_middle %>% dplyr::select(q22) %>%
  colSums() %>% 
  as_data_frame() %>% 
...
TODO
Look at whether demographic features correlate with the presence of arts sequences.
...
One would hope that schools with poor, minority students received at least as much external funding as the wealthier and whiter schools.
We can also look at whether there is an association between academic performance and external funding.
The direction of any relationship between academic performance and funding would be ambiguous. It is possible that funding flows to overperforming schools as their status attracts donors to them or that the additional funding causes increased academic performance.
To tease this relationship apart we would need to identify an instrumental variable.  
...
Government grants are the most common source of non-DOE funding, followed by cultural organizations, and PTA. Educational associations and local business are the least common sources of arts funding.  

Does poverty predict any of the different sources of funding?  

```{r art funding poverty}
for (i in q36) {
  temp <- df %>%
    dplyr::select(i, perc_pov_2017)
  temp.glm <- glm(unlist(temp[,1])~unlist(temp[,2]), family='binomial')
  print(i); print(summary(temp.glm))
}
```

Poverty has a statistically significant negative coefficient for PTA/PA funding at a p-value of 0.001 and local arts councils at a p-value of 0.01. Could this effect be clarified by controlling for additional features?
...
Government grants have a statistically significant positive coefficient for percentage of students in poverty, p-value 0.01, and a negative coefficient for percentage of black students, p-value 0.001.
PTA funding has a statistically significant negative coefficient for percentage of students in poverty and black students, both at p-values of 0.001.
These results tell us that funding from government grants tends to flow to schools with poor students and not to those with black students. PTA funding does not tend to flow to poor schools or schools with black students.
The finding about government grants going to poor schools is hopeful, since these schools are evidently not receiving much funding from PTAs. On the other hand, that government grants are not going to schools with black students is a source of concern. We have not shown that there is a causal relationship, but this correlation should be investigated further.
Our findings about the direction of PTA funding to richer and whiter schools is should not be surprising. An active PTA can be the difference between whether a school can afford to purchase newly published textbooks or has to stick with science books from the 2000s. The current system will allocate funding from parent donors to their own children's schools, so schools serving wealthier students will benefit most from this source of funding.  
...
for (i in q36) {
  ...
  temp.glm <- glm(unlist(temp[,1])~unlist(temp[,2])+unlist(temp[,3])+unlist(temp[,4])+unlist(temp[,5]), family='binomial')
  print(i); print(summary(temp.glm))
}
```

Now we have tried controlling for poverty, percentage of black students, total enrollment, and academic performance.
Funding from government grants still has a statistically significant negative coefficient for percentage of black students, at a p-value of 0.01. The coefficient for students in poverty is no longer significant. 
PTA funding has a statistically significant negative coefficient for poverty at a p-value of 0.001. The coefficient for percentage of black students is no longer significant.
Funding from arts councils has a statistically significant positive coefficient for student population at a p-value of 0.01.  
That the association between government grants and percentage of black students continues to be a negative one is unfortunate news. As is the lack of an association with poor student populations, whom we would have hoped to receive more funding from this source.
For PTA funding, controlling for the additional features has removed the significant coefficient for all but the wealth of the student population. As we mentioned earlier, this association is a direct by-product of the PTA system and to be expected. 
Funding from arts councils has a newly significant association with the size of a school's population. This is interesting because we can postulate reasons for this association, such as greater resources available to large schools allowing them to apply to these councils for funding or greater visibility for large schools to attract council attention. These two mechanisms could be addressed by educating small schools about seeking funding from arts councils or educating the councils about the needs of small schools. It is also possible that the reason is structural, large schools cover more and larger local arts councils which results in their being the recipient of more funding from this source.  

## Part Ten

### Parental Involvement

Schools were asked about whether parents participated in their arts programs in one of the following ways: attending events, volunteering, donating supplies or other. We can hypothesize that the same schools as receive PTA funding are the ones that have a lot of parental involvement.  
...
Parental participation is most commonly found in the form of attendance at school events. This is the least time-intensive and most emotionally rewarding of the options and fewer parents volunteer or donate art materials.  
...
There is a statistically significant negative coefficient for the relationship between poverty and parental volunteering in arts programs at a p-value of 0.01, even after controlling for school size and academic performance.  

Is there a relationship between poverty and donating arts materials, Q39_3?

```{r parental inv donation}
df %>% 
  dplyr::select(Q39_3, perc_pov_2017, total_enrollment_2017, perc_34_all_2018) %>% 
  glm(Q39_3~., data=., family='binomial') %>% 
  summary()
```

There is a statisically significant negative coefficient for the relationship between poverty and donating arts materials at a p-value of 0.001.  
We can explicitly look at the correlation between schools receiving funding from their PTAs and parental involvement in the arts.
...
Of the different manners in which parents can become involved with arts programs directly, PTA funding is most closely associated with donations. This makes sense if we consider that schools reporting parental involvement through donations have taken these into consideration when answering the question about PTA funding. It is possible to consider these sources of funding as distinct, but it is not clear that school administrators would agree.  
